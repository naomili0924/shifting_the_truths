...
hf download "${SDXL_REPO}" --repo-type model --local-dir "${SDXL_DIR}"

echo "Restoring Hunyuan-DiT -> ${HUNYUAN_DIR}"
hf download "${HUNYUAN_REPO}" --repo-type model --local-dir "${HUNYUAN_DIR}"
...
if [ "${SKIP_INPAINT:-0}" != "1" ]; then
  ...
  hf download "${INPAINT_REPO}" --repo-type model --local-dir "${INPAINT_DIR}" \
